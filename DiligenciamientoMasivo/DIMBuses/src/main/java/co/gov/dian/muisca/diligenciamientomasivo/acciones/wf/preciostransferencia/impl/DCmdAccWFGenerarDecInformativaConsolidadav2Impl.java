...
import co.gov.dian.muisca.diligenciamientomasivo.acciones.wf.preciostransferencia.DCmdAccWFGenerarDecInformativaConsolidadav2;
...
import co.gov.dian.muisca.diligenciamientomasivo.general.preciostransferencia.DHelperExpedienteGeneracion130v2;
 ...
 * @version 1.0
 */
public class DCmdAccWFGenerarDecInformativaConsolidadav2Impl extends
  DCmdAccWFGenerarDecInformativaConsolidadav2 {

  private static final Logger logger = Logger.getLogger(
        DCmdAccWFGenerarDecInformativaConsolidadav2Impl.class);
  ...
  private final String ERROR_NO_DOC_130_ANTERIOR =
    ...
    private final String ERROR_DOC_INICIAL_EXISTE = "Ya existe una declaracion INICIAL para el aï¿½o gravable. " +
    "No es posible presentar la declaraciï¿½n ";
...
//Casillas del 120 para matriz
  ...
  //Helpers
  DHelperExpedienteGeneracion130v2 manejadorExpedientes = null;

  public DCmdAccWFGenerarDecInformativaConsolidadav2Impl() {
  }

    /**
     * Este metodo valida que no se procesen dos cargas del mismo aï¿½o.
     *
     * @return boolean
  ...
  private DHelperExpedienteGeneracion130v2 getManejadorExpedientes() {
  ...
  protected void ejecutarComandoSinTransaccion() {
    logger.info("Inicio WF Decl. Inf. Precios de Transferencia Consolidada v2");
    ...
    try {
      if (procesarValido) {
        ...
        if(annoSolicitud >= 2012)
        	version120 = 4;
        //if(annoSolicitud == 2013)
           // isPeriodo = true;
        
        logger.info("El año de solicitud es :" + annoSolicitud);
        logger.info("La versión del formato del 120 es :" + version120);
        logger.info("El año gravable del 120 es :" + ANNO_GRAV);
        ...
        logger.info("Existe el siguiente número de identificadores " + identificadores.size());
        ...
        if (identificadores != null && !identificadores.isEmpty()) {
        	...
        	logger.info("Existe el siguiente nï¿½mero de identificadores " + identificadores.size());
        	
        	Iterator itTemp = identificadores.iterator();
        	
        	while(itTemp.hasNext()){
        		DIdentificadorDoc ident = (DIdentificadorDoc)itTemp.next();
        		logger.info("Declaración 120 encontrada: " + ident.getIdDocumento());
        	}
  ...
  private Long notificarResponsable(DSolicitudIngresoTO miSolicitud,
      ...
      if (procesarValido) {
          ...
          urlTarea = "/WebDiligenciamientomasivo/DefPreciosTransConsolidadav2.faces";
      }
  ...
  private IDDocumento dehomologar(IDDocumento documento) throws DExcepcion {
    ...
    if (logDocumento.getConteo() > 0) {
      String mensaje = "El Documento " + documento.getId() + " con repeticiï¿½n " +
