...
import co.gov.dian.muisca.diligenciamientomasivo.acciones.wf.preciostransferencia.DCmdAccWFGenerarDecInformativaConsolidadav8;
...
import co.gov.dian.muisca.diligenciamientomasivo.general.preciostransferencia.DHelperExpedienteGeneracion130v8;
...
import co.gov.dian.muisca.rut.acciones.DCmdAccConsRegistroRut;
 ...
 * @version 1.0
 */
public class DCmdAccWFGenerarDecInformativaConsolidadav8Impl extends
  DCmdAccWFGenerarDecInformativaConsolidadav8 {

  private static final Logger logger = Logger.getLogger(
        DCmdAccWFGenerarDecInformativaConsolidadav8Impl.class);
  ...
  private static final int CASILLA_TOTAL_OPERACIONES_INGRESO_OBLIGADO = 38;
  private static final int CASILLA_TOTAL_OPERACIONES_EGRESO_OBLIGADO = 39;
  private static final int CASILLA_ACTIVO_MOVIMIENTO_DEBITO_OBLIGADO = 40;
  private static final int CASILLA_ACTIVO_MOVIMIENTO_CREDITO_OBLIGADO = 41;
  private static final int CASILLA_ACTIVO_SALDO_OBLIGADO = 42;
  private static final int CASILLA_PASIVO_MOVIMIENTO_DEBITO_OBLIGADO = 43;
  private static final int CASILLA_PASIVO_MOVIMIENTO_CREDITO_OBLIGADO = 44;
  private static final int CASILLA_PASIVO_SALDO_OBLIGADO = 45;
...
//Casillas del 120 para matriz
  ...
  private static final int VERSION_FORMATO = 8;
  ...
  //Helpers
  DHelperExpedienteGeneracion130v8 manejadorExpedientes = null;

  public DCmdAccWFGenerarDecInformativaConsolidadav8Impl() {
  }
  ...
  private DHelperExpedienteGeneracion130v8 getManejadorExpedientes() {
  ...
  protected void ejecutarComandoSinTransaccion() {
    logger.info("Inicio WF Decl. Inf. Precios de Transferencia Consolidada v8");
  ...
  protected void ejecutarComandoWF() {
    try {
      if (procesarValido) {
        ...
        ocurrencia.getValorCasilla(CASILLA_TOTAL_OPERACIONES_INGRESO).
          setValor(new
          Long(totalOperacionesIngreso));
        ocurrencia.getValorCasilla(CASILLA_TOTAL_OPERACIONES_EGRESO).
          setValor(new
          Long(totalOperacionesEgreso));
        ocurrencia.getValorCasilla(CASILLA_ACTIVO_MOVIMIENTO_DEBITO).
          setValor(new
          Long(totalActivoMovimientoDebito));
        ocurrencia.getValorCasilla(CASILLA_ACTIVO_MOVIMIENTO_CREDITO).
          setValor(new
          Long(totalActivoMovimientoCredito));
        ocurrencia.getValorCasilla(CASILLA_ACTIVO_SALDO).setValor(new
          Long(totalActivoSaldo));

        ocurrencia.getValorCasilla(CASILLA_PASIVO_MOVIMIENTO_DEBITO).
          setValor(new
          Long(totalPasivoMovimientoDebito));
        ocurrencia.getValorCasilla(CASILLA_PASIVO_MOVIMIENTO_CREDITO).
          setValor(new
          Long(totalPasivoMovimientoCredito));
        ocurrencia.getValorCasilla(CASILLA_PASIVO_SALDO).setValor(new
...
// Manejo de individual para matriz
        ...
        DCmdAccConsRegistroRut consRegistroRut = (DCmdAccConsRegistroRut)
          getAccion("rut.DCmdAccConsRegistroRut");
        consRegistroRut.inicializarConsultarPorNIT(nit.longValue());
        consRegistroRut.ejecutar();
        to = consRegistroRut.getRegistroRut();
        idenDocTo.setIdPerOrganizacion(to.getPK().getIdePersonaRut());
        idenDocTo.setIdFormato(new Integer(NUM_FORMATO_INDIVIDUAL));

        //Igual que en la validaci�n se debe verificar el a�o,
        //como el identificador no soporta a�o gravable se quema la versi�n
        /*int annoSolActual = solicitud.getSolicitudAtt().getAnioVigencia().
                            intValue();*/
        ...
        Integer ANNO_GRAV = ocurrencia.getValorCasilla(CASILLA_ANIO).getIntWrapper();
        ...
        if(annoSolicitud >= 2009)
        	version120 = 1;
        if(annoSolicitud == 2010)
            isPeriodo = true;
        ...
        if (idenDoc != null) {
          ...
          if(version120 == 1)
          {	
        	  		ocurrencia.getValorCasilla(CASILLA_TOTAL_OPERACIONES_INGRESO_OBLIGADO).
        	  		  setValor(ocurrenciaInd.getValorCasilla(CASILLA_28_120).
                      getValorEntero());
                    ocurrencia.getValorCasilla(CASILLA_TOTAL_OPERACIONES_EGRESO_OBLIGADO).
                      setValor(ocurrenciaInd.getValorCasilla(CASILLA_29_120).
                      getValorEntero());
                    ocurrencia.getValorCasilla(CASILLA_ACTIVO_MOVIMIENTO_DEBITO_OBLIGADO).
                      setValor(ocurrenciaInd.getValorCasilla(CASILLA_30_120).
                      getValorEntero());
                    ocurrencia.getValorCasilla(CASILLA_ACTIVO_MOVIMIENTO_CREDITO_OBLIGADO).
                      setValor(ocurrenciaInd.getValorCasilla(CASILLA_31_120).
                      getValorEntero());
                    ocurrencia.getValorCasilla(CASILLA_ACTIVO_SALDO_OBLIGADO).
                      setValor(ocurrenciaInd.getValorCasilla(CASILLA_32_120).
                      getValorEntero());
                    ocurrencia.getValorCasilla(CASILLA_PASIVO_MOVIMIENTO_DEBITO_OBLIGADO).
                      setValor(ocurrenciaInd.getValorCasilla(CASILLA_33_120).
                      getValorEntero());
                    ocurrencia.getValorCasilla(CASILLA_PASIVO_MOVIMIENTO_CREDITO_OBLIGADO).
                      setValor(ocurrenciaInd.getValorCasilla(CASILLA_34_120).
                      getValorEntero());
                    ocurrencia.getValorCasilla(CASILLA_PASIVO_SALDO_OBLIGADO).
                      setValor(ocurrenciaInd.getValorCasilla(CASILLA_35_120).
                      getValorEntero());
          }
...
// Se setean estos valores por que son obligatorios y tipo entrada 5 esta estricto S
  ...
  private Long notificarResponsable(DSolicitudIngresoTO miSolicitud,
      ...
      if (procesarValido) {
          ...
          urlTarea = "/WebDiligenciamientomasivo/DefPreciosTransConsolidadav8.faces";
      }
      ...
      // Anexar el numero de solicitud al mensaje
      //msjNotificacion =  msjNotificacion + documentoCargaPK.getIdeDocumento().toString();
