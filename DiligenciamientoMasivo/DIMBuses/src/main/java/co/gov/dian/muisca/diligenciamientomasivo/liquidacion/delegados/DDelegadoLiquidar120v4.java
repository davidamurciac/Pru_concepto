...
public class DDelegadoLiquidar120v4 extends DDelegadoLiquidar {
	...
	public String getDescripcion() {
		return "Delegado que implementa la Liquidación para el Formulario 120 versión 4.";
	}
	 ...
	 * @throws DExcepcion
	 */
	private void efectuarLiquidacion() throws DEntradaSalidaExcepcion,
			DExcepcion {

		// Obtiene información de la declaración.

		IDOcurrencia ocurrenciaDoc120 = docLiquidacion.getGrupos()
				.getGrupo(GRUPO_DOC_LIQUIDACION)
				.getOcurrencia(IDLiquidacion.OCURRENCIA_DOC_LIQUIDACION);
		int valCasSancion = ocurrenciaDoc120.getValCasillaNeg(
				IDCasillasFormularios.SANCION).getId();
		 ...
		 * frStr = frStr!=null?frStr:"false"; boolean isFraccion =
		 * Boolean.getBoolean(frStr);
		 */
		...
		/*
		 * if(!isFraccion){ DCmdSrvLiquidarCuota srvLiquidarCuota =
		 * (DCmdSrvLiquidarCuota) getServicio(
		 * "entradasalida.liquidacion.DCmdSrvLiquidarCuota");
		 * srvLiquidarCuota.inicializar(metaLiquidacion, nit,
		 * declaracionPagoSancion, declaracionPagoImpuesto);
		 * srvLiquidarCuota.ejecutar();
		 * DCalculosLiqF490.asignarValoresDocLiquidado( docLiquidado,
		 * srvLiquidarCuota.getPagoSancion(),
		 * srvLiquidarCuota.getPagoInteresMora(),
		 * srvLiquidarCuota.getPagoImpuesto()); }else{
		 */
		DCalculosLiqF490.asignarValoresDocLiquidado(docLiquidado,
				declaracionPagoSancion, pagoInteresMora,
