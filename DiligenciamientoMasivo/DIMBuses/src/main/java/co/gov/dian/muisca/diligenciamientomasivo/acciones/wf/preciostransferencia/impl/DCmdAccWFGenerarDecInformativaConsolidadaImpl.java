...
import co.gov.dian.muisca.diligenciamientomasivo.acciones.wf.preciostransferencia.
  DCmdAccWFGenerarDecInformativaConsolidada;
...
import co.gov.dian.muisca.entradasalida.formatos.*;
import co.gov.dian.muisca.entradasalida.acciones.*;
import co.gov.dian.muisca.rut.servicios.*;
...
import co.gov.dian.muisca.diligenciamientomasivo.general.preciostransferencia.
  DHelperExpedienteGeneracion130v6;
 ...
 * @version 1.0
 */
public class DCmdAccWFGenerarDecInformativaConsolidadaImpl extends
  DCmdAccWFGenerarDecInformativaConsolidada {
  ...
  private static final int CASILLA_DOCUMENTO_RADICACION = 46;
  ...
  private DHelperExpedienteGeneracion130v6 manejadorExpedientes = null;

  public DCmdAccWFGenerarDecInformativaConsolidadaImpl() {
  }
    ...
    try {
      ...
      if (procesarValido) {
        ...
        DCmdSrvConsLstSolicitudArchivo srvConsLstConsArchivo = (
          ...
          "entradasalida.cargamasiva.DCmdSrvConsLstSolicitudArchivo");
  ...
  protected void ejecutarComandoWF() {
    try {
      if (procesarValido) {
        ...
        ocurrencia.getValorCasilla(CASILLA_TOTAL).setValor(new Long(0));

        for (int i = 36; i < 44; i++) {
          ocurrencia.getValorCasilla(i).setValor(new Long(0));
        }
      ...
      LOGGER.error("Error al ejecutar ejecutarComandoWF(), ",
        ex);
      isFinalizado = true;
      isOk = false;
    }
  ...
  private DIdentificadorDoc salvarDocumento(IDDocumento doc) throws
    ...
    DCmdAccCrearDocumentoESTemporal dCmdSrvCrearDocumentoES =
      (DCmdAccCrearDocumentoESTemporal) getAccion(
      "entradasalida.DCmdAccCrearDocumentoESTemporal");

    if (solicitudIngresoTO.getSolicitudAtt().getCodConcepto().intValue() !=
      CONCEPTO_CORRECCION) {
      dCmdSrvCrearDocumentoES.inicializarGuardarSinId(doc,
        IDFormato.
      ...
      dCmdSrvCrearDocumentoES.inicializarGuardarSinId(doc,
        IDFormato.
        ENTRADA_DILIGENCIAMIENTO_ADMITIVO,
  ...
  private Long notificarResponsable(DSolicitudIngresoTO miSolicitud,
    ...
    if (procesarValido) {
      ...
      urlTarea = "/WebDiligenciamientomasivo/DefPreciosTransConsolidada.faces";
    }
  ...
  private DHelperExpedienteGeneracion130v6 getManejadorExpedientes() {
